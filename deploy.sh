...
if [ -z "$VM_IP" ]; then
    echo "VM_IP is not set. Exiting."
    exit 1
fi

echo "Starting deployment..."

echo "Syncing files to $VM_USER@$VM_IP:$TARGET_DIR..."
rsync -avz --exclude='.git/' --exclude='node_modules/' --exclude='venv/' ./ "$VM_USER@$VM_IP:$TARGET_DIR/" || {
    echo "File sync failed. Exiting."
    exit 1
}

echo "Connecting to $VM_USER@$VM_IP to start deployment..."
